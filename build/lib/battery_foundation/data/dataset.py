    ...
    def _process_battery_data(self, battery_data: Dict[str, Any]) -> List[Dict[str, torch.Tensor]]:
        ...
        for cycle in cycle_data:
            try:
                ...
                if len(voltage) < self.patch_len * self.patch_num:
                    continue
                
                # Combine V and I as the two variables
                vc_data = np.stack([voltage, current], axis=-1)  # Shape: (time_steps, 2)
                
                # Create patches
                patches = self._create_patches(vc_data)
                if patches is None:
                    continue
                
                # Calculate capacity degradation (Q) for each cycle
                ...
                # Create Q data (QdQc) 
                qdqc = np.array([q_discharge, q_charge])
                qdqc_patches = np.tile(qdqc, (self.patch_num, 1))
                
                # Create time and mask tensors
                t = torch.arange(self.patch_num + 1).float()  # Include start token
                tm = torch.ones(self.patch_num + 1, self.patch_num + 1)  # Triangular mask
                pm = torch.ones(self.patch_num)  # Patch mask
                ...
                logger.debug(f"Error processing cycle: {e}")
                continue
